...
public class ContactlistFragment extends Fragment {
	...
	public void deleteContact(final User tobeDeleteUser) {
		...
		String currentName = SuperWeChatApplication.getInstance().getUserName();
		final OkHttpUtils2<Result> utils = new OkHttpUtils2<Result>();
		utils.setRequestUrl(I.REQUEST_DELETE_CONTACT)
				.addParam(I.Contact.USER_NAME,currentName)
...
//            contactList.add(0, users.get(Constant.CHAT_ROOM));
	...
	UpdateContactReceiver mUpadeteContactReceiver;
	private void updateContactListener(){
		mUpadeteContactReceiver = new UpdateContactReceiver();
		IntentFilter filter = new IntentFilter("update_contact_list");
		getActivity().registerReceiver(mUpadeteContactReceiver,filter);
	}

	@Override
	public void onDestroyView() {
		super.onDestroyView();
		getActivity().unregisterReceiver(mUpadeteContactReceiver);
	}
}
